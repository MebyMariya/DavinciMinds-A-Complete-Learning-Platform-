...
EYE_AR_THRESH = 0.2 # Eye Aspect Ratio threshold below which eyes are closed
EYE_AR_CONSEC_FRAMES = 3 # No. of consecutive frames of eye closure to trigger warning
MOUTH_AR_THRESH = 30
...
def parent_portal():
    if request.method == 'POST':
            ...
            try:
                with sqlite3.connect(db_name) as conn:
                    ...
                    return [
                        {
                            "date": row[0],
                            "score": int(row[1].split('/')[0])  # Extract numerator (raw score)
                        }
                        for row in rows
                    ][::-1]
            except sqlite3.Error as e:
                    ...
                    attention_list = [
                        {
                            ...
                            "attention": row[2]  # Store attention_lost_frame for display
                        }
                        for row in rows
                    ][::-1]

                    # Calculate Focus Level and Engagement for the latest session
                    if rows:
                        latest_session = rows[0]  # Latest session is the first after reversing
                        login_time = datetime.fromisoformat(latest_session[0])
                        logout_time = datetime.fromisoformat(latest_session[1])
                        attention_lost_frames = float(latest_session[2])  # Convert to float for calculation

                        session_duration_seconds = (logout_time - login_time).total_seconds()
                        global fps
                        total_frames = session_duration_seconds * fps if fps > 0 else 1

                        # Focus Level: Percentage of time attentive (inversely proportional to attention_lost_frames)
                        focus_level = max(0, ((total_frames - attention_lost_frames) / total_frames) * 100)

                        # Engagement: A simplified metric based on focus level (could be adjusted further)
                        # Engagement decreases as attention_lost_frames increases
                        engagement = focus_level  # For simplicity, engagement mirrors focus level
                        # Optionally, adjust engagement with additional factors (e.g., weight attention loss more heavily)
                        if attention_lost_frames > total_frames * 0.5:  # If attention lost for more than 50% of the session
                            engagement *= 0.5  # Reduce engagement significantly

                        return attention_list, round(focus_level, 2), round(engagement, 2)
                    return attention_list, 0, 0  # Default to 0 if no data
            except sqlite3.Error as e:
        ...
        return render_template(
            ...
            engagement=engagement  # Pass engagement to the template
        )
...
def video_to_text(video_path):
    ...
    if not clip.audio:
        raise ValueError("Error: Video has no audio track!")

    clip.audio.write_audiofile(audio_path)
    
    if not os.path.exists(audio_path) or os.path.getsize(audio_path) == 0:
        raise ValueError("Error: Extracted audio file is empty!")

    result = model.transcribe(audio_path)
    text = result.get('text', '')
    
    if not text:
        raise ValueError("Error: Whisper transcribed an empty file!")
...
def generate_summary(text):
    ...
    response = chat_session.send_message("Summarize the given text into 300 words maximum with headings and bullet points, using the format: # Main Topic, ## Subheading, - Bullet point: " + text)
...
@app.route('/notes')
def notes():
    print("Session Filepath:", session.get("filepath", "No file found"))
    
    if "filepath" not in session or not os.path.exists(session["filepath"]):
        return "Error: Video file not found!", 400

    text = video_to_text(session["filepath"])
    print("Extracted Text:", text)

    if not text.strip():
        return "Error: Transcription is empty!", 400
...
def generate_notes(text):
    ...
    response = chat_session.send_message(
            "From the given text Generate notes with headings and subheadings : " + text
        )
...
def autonotes():
    ...
    try:
        autonote = generate_notes(text)
        with open("notes.txt", "w", encoding="utf-8") as file:
            file.write(autonote)
        return render_template('notes2.html', notes2=autonote)
    except Exception as e:
...
def youtube_mindmap():
    try:
        ...
        # Extract video_id from session['current_url']
        current_url = session.get('current_url')
        if not current_url:
            print("Warning: No current_url in session, redirecting to search")
            return redirect(url_for('youtubesearch'))
        video_id = current_url.split('v=')[-1]  # Extract video ID from URL (e.g., "VIDEO_ID" from "https://www.youtube.com/watch?v=VIDEO_ID")
        
        generate_and_save_detailed_mindmap(text_content)
        return render_template('mindmap.html', 
                             image_url=url_for('static', filename='mindmap_detailed.jpg'),
...
def youtubeautonotes():
    ...
    return render_template('notes2.html', notes2=autonotes, video_id=video_id)
...
def generate_mcqs(text):
    try:
        ...
        Provide the correct answer for each question. Return the result as a valid JSON array in this format:
        [
            {{"question": "Question text", "options": ["a", "b", "c", "d"], "answer": "correct_option"}},
            ...
        ]
...
def submit_quiz():
    ...
    score = sum(1 for i, mcq in enumerate(mcqs) if answers.get(f'q{i}', '').strip() == mcq['answer'].strip())
    total = len(mcqs)
    ...
    for i, mcq in enumerate(mcqs):
        user_answer = answers.get(f'q{i}', 'Not answered')
...
def logout():
    ...
    at = 0
    session.clear()
    return redirect(url_for('login', error="Logged out successfully"))
